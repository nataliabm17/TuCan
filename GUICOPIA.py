...
def main():
    ...
    v_start = False  # 16 uses to determine if program starts with data recopilation
    v_document = None  # 17 uses to access csv file
    v_skip = -1  # 18

    v_gps_time_minute = 0  # 19
    v_gps_time_seconds = 0  # 20

    v_variables = [v_time, v_packet_count, v_altitude, v_pressure, v_temp, v_voltage, v_gps_time_hour, v_gps_latitude,
                   v_gps_longitude, v_gps_altitude, v_gps_sats, v_pitch, v_roll, v_blade_spin_rate, v_software_state,
                   v_launch, v_start, v_document, v_skip, v_gps_time_minute, v_gps_time_seconds]
    ...
    while True:  # Program's Loop
        ...
        if v_variables[16]:  # starts transmission
